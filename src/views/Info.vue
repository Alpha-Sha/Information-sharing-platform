<template>
  <div class="home">
    ...
    <el-button type="primary" @click="createInfoVisible = true">发布信息</el-button>
    <div class="inputWrap">
    ...
    <div class="menuWrap">
      <div v-for="(item, index) in menuList" :key="index" @click="menuIndex = index">
        <div v-if="operator || (index != 1 && index != 2)" class="menuItem" :class="index == menuIndex && 'active'">{{ item.name }}</div>
      </div>
    </div>
    ...
    <div class="list">
      <div class="topInfo" v-for="(item, index) in getMarketList" :key="index">
        <div>
          <img v-if="item.imgUrl" :src="item.imgUrl" :onerror="defaultImg" alt="" class="infoImg" />
        ...
        <div>
          <el-button type="primary" @click="handleAuditInfo(item)" v-if="item.status == 1 && operator">审核通过</el-button>
          <el-button type="danger" @click="handleCloseInfo(item)" v-if="(item.status == 1 || item.status == 2) && (item.publisher == account || operator)">关闭信息 </el-button>
        </div>
      </div>
    </div>

    <el-dialog title="发布信息" :visible.sync="createInfoVisible" width="500px">
      <el-form :model="form">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleNewInfo">确定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="View advertising details" :visible.sync="adDetailVisible" width="500px">
      <img :src="adDetail.imgUrl" :onerror="defaultImg" alt="" class="infoImg" />
...
export default {
  ...
  data() {
    return {
      ...
      inputAddress: '',
      createInfoVisible: false, //发布信息弹窗
      marketList: [],
      menuList: [
        {
          name: '上架',
  ...
  methods: {
    ...
    // 上架信息
    async handleAuditInfo(item) {
      const marketContract = getMarketContract(this.provider);
      try {
        const gas = await marketContract.methods.auditInfo(item.id, 2).estimateGas({ from: this.account });
    ...
    async getMarket() {
      const marketContract = getMarketContract();
      const length = await marketContract.methods.infoLength().call();
      const p = [];
      for (let i = 0; i < length; i++) {
        p.push(marketContract.methods.infos(i).call());
      }
    ...
    async handleNewInfo() {
      ...
      try {
        const gas = await marketContract.methods.newInfo(imgUrl, describe).estimateGas({ from: this.account });
        await marketContract.methods
          .newInfo(imgUrl, describe)
          .send({ from: this.account, gas: gasProcessing(gas) })
          .on('transactionHash', hash => {
            this.createInfoVisible = false;
            this.form = {
    ...
    // 下架关闭信息
    async handleCloseInfo(item) {
      const marketContract = getMarketContract(this.provider);
      try {
        const gas = await marketContract.methods.closeInfo(item.id).estimateGas({ from: this.account });
        await marketContract.methods
          .closeInfo(item.id)
          .send({ from: this.account, gas: gasProcessing(gas) })
          .on('transactionHash', hash => {})
